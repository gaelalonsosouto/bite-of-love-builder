...
import { bloquesQueryOptions, t } from "@/lib/content";

export function SiteFooter() {
  const { data: bloques } = useQuery(bloquesQueryOptions);
  const b = bloques ?? new Map();
  return (
    <footer className="relative z-10 bg-ink text-cream border-t border-border mt-32">
      <div className="container-x py-20 grid gap-12 md:grid-cols-3">
        <div>
          ...
          <p className="mt-3 text-sm text-cream/60">{t(b, "footer_tagline", "Muerde el amor. Smash burgers en A Coruña.")}</p>
          <div className="mt-6">
            <SocialLinks />
          </div>
        </div>
        <div className="text-sm space-y-2">
          ...
          <p>{t(b, "footer_direccion_1", "C. Pondal, 1, Bajo F")}</p>
          <p>{t(b, "footer_direccion_2", "15004 A Coruña")}</p>
          <a href="tel:+34881336926" className="inline-block mt-1 hover:text-tomato transition-colors hover:scale-105">{t(b, "footer_telefono", "+34 881 33 69 26")}</a>
        </div>
        <div className="text-sm space-y-2">
